...
namespace SANS.Common.Cache
{
    public class CacheContext : ICacheContext
    {
        ...
        public override bool SetSliding<T>(string key, T t, TimeSpan expire)
        {
            var obj = Get<T>(key);
            if (obj != null)
            {
            ...
            _objCache.Set(key, t, new MemoryCacheEntryOptions().SetSlidingExpiration(expire));   //相对过期时间
